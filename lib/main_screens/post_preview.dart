...
class _PostPreviewScreenState extends State<PostPreviewScreen> {
  ...
  Future<String> _uploadToCloudinary() async {
    try {
      ...
      if (!exists) throw Exception('Image file does not exist: ${widget.mediaFile.path}');

      print('Uploading to Cloudinary: ${widget.mediaFile.path}');
      final uri = Uri.parse('https://api.cloudinary.com/v1_1/$_cloudName/image/upload');
      final request = http.MultipartRequest('POST', uri)
        ..fields['upload_preset'] = _uploadPreset
        ..files.add(await http.MultipartFile.fromPath('file', widget.mediaFile.path));
      ...
      if (response.statusCode != 200) {
        throw Exception('Cloudinary upload failed: ${response.statusCode} - ${jsonResponse['error']?['message'] ?? responseBody}');
      }
  ...
  Future<void> _uploadPost() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        Navigator.popUntil(context, (route) => route.isFirst); // Return to MainHomeScreen
